...
# extracts data from single game
def scrape_data(page_soup):
    ...
    # team 1 results
    score = page_soup.select_one('.game-strip.game-package.nfl.post .team.away .score').text
    opponent_score = page_soup.select_one('.game-strip.game-package.nfl.post .team.home .score').text
    ...
    first_team_data = [game_id, home_or_away, team, qb_with_most_yds_name, qb_with_most_yds_qbr, team_qb_c, team_qb_att, team_qb_yds, team_qb_avg, team_qb_td, team_qb_int, team_qb_sack_number, team_qb_sack_yds, team_qb_rtg, \
                        ...
                        fum, lost, rec, sacks, ints, score, opponent_score, result, record_after_game]
    first_team_data = [str(i) for i in first_team_data]


    #========================
    # team 2 is the home team
    home_or_away = page_soup.select('#gamepackage-matchup-wrap .record')[1].text
    home_or_away = home_or_away.split()
    # if the game is played at a neutral location, this list will only have length 2, else it will be specefied in home_or_away[2]
    if len(home_or_away) < 3:
        home_or_away = 'Neutral'
    ...
    # team 2 results
    score = page_soup.select_one('.game-strip.game-package.nfl.post .team.home .score').text
    opponent_score = page_soup.select_one('.game-strip.game-package.nfl.post .team.away .score').text
    if int(score) > int(opponent_score):
        result = 'W'
    else:
        result = 'L'
    ...
    # record after game
    record_after_game = page_soup.select('#gamepackage-matchup-wrap .record')[1].text
    record_after_game = record_after_game.split(',')[0]
    ...
    return first_team_data, second_team_data

years = [FIRST_YEAR + i for i in range(LAST_YEAR - FIRST_YEAR + 1)]
weeks = [i for i in range(1, 17)]

total_data = []
# iterates through every week of the years specified
for year in years:
    print("\n\nyear: ", year)
    for week in weeks:
        print("week: ", week)
        url = 'https://www.espn.com/nfl/scoreboard/_/year/' + str(year) + '/seasontype/2/week/' + str(week)

        # load page with every weekly game
        driver = webdriver.Chrome()
        driver.get(url)
        res = driver.execute_script('return document.documentElement.outerHTML')
        driver.quit()

        # extract url of each weekly game
        page_soup = soup(res, "html.parser")
        games = page_soup.select('#events .scoreboard.football.final.js-show')
        for game in games:
            game_id = game['id']
            url = 'https://www.espn.com/nfl/boxscore?gameId=' + game_id
            ...
            # scrapes data from single game
            first_team_data, second_team_data = scrape_data(page_soup)
            # print("\n\nfirst team data = ", first_team_data)
            # print("second team data = ", second_team_data)

            # add the year of season and week
            first_team_data.insert(1, year)
            first_team_data.insert(2, week)
            second_team_data.insert(1, year)
            second_team_data.insert(2, week)

            # appends data to list
            total_data.append(first_team_data)
            total_data.append(second_team_data)
            np.savetxt('nfl_box_score_data/' + str(FIRST_YEAR) + '-' + str(LAST_YEAR) + '.csv', total_data, delimiter = ',', fmt = '%s')